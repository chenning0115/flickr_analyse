...
public class Kmeans {
	...
	int k = 10;
	int maxnum = 30;
	private double interval = 0.001d;//about 10.8m*10.8m in the earth
	private boolean IsCanopy = false;
	double T1;
	double T2;
	public Kmeans(FlickrPoint[] ori_points,int input_k,int max)
	{
	...
	public Kmeans(FlickrPoint[] ori_points,int input_k,double interval,int max)
	{
		this.points = ori_points;
		this.k = input_k;
		//initial
	...
	public Kmeans(FlickrPoint[] ori_points,int max,double t1,double t2)
	{
		this.points = ori_points;
		//this.k = input_k;
		//initial
	...
	private boolean checkcenter()
	{
		...
		if(dissquare>=0&&dissquare<interval) return true;
		else {
			return false;
		}
	...
	public void kmeanstoshp(String path)
	{
		new ToSHP(points, result,k, path+"_kmeans_class_").Toshapefile();
		int[] center_result = new int[k];
		for(int i = 0;i<k;i++) center_result[i] = i;
		new ToSHP(cur_centers, center_result, k, path+"_kmeans_center_").Toshapefile();
	}
